...
export class TokenboundController {
   ...
   @Post("/increase-point")
   async increasePoint(
      @Body() { tokenboundAddress, point }: { tokenboundAddress: string , point : number}
   ) {
      try {
         let tokenboundAccount: TokenboundEntity | null =
   ...
   async refreshOwner(@Body() { tokenId }: { tokenId: number }) {
      ...
      try {
         ...
         const newTokenbound = await this.tokenboundService.save(tokenbound);
         return newTokenbound;
      } catch (error) {
         throw new HttpError(
            StatusCodes.INTERNAL_SERVER_ERROR,
   ...
   async refreshListingStatus(@Body() { tokenId }: { tokenId: number }) {
      ...
      try {
         ...
         console.log(address)
         console.log(config.MARKET_CONTRACT_ADDRESS)
